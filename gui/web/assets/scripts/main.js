...
class Broadcast extends BaseElement {
  ...
  async sendPrivate() {
    ...
    const ok = this.checkInputs(
      this.privateMessageTarget,
      this.privateRecipientsTarget
    );
    if (!ok) {
      return;
    }
...
class DataSharing extends BaseElement {
  ...
  async download() {
    ...
    const addr = this.peerInfo.getAPIURL(
      "/datasharing/download?key=" + metahash
    );
...
class Search extends BaseElement {
  ...
  async searchAll() {
    ...
    const ok = this.checkInputs(
      this.searchAllPatternTarget,
      this.searchAllBudgetTarget,
      this.searchAllTimeoutTarget
    );
    if (!ok) {
      return;
    }
  ...
  async searchFirst() {
    ...
    const ok = this.checkInputs(
      this.searchFirstPatternTarget,
      this.searchFirstInitialBudgetTarget,
      this.searchFirstFactorTarget,
      this.searchFirstRetryTarget,
      this.searchFirstTimeoutTarget
    );
    if (!ok) {
      return;
    }
...
class Naming extends BaseElement {
  ...
  async resolve() {
    ...
    const addr = this.peerInfo.getAPIURL(
      "/datasharing/naming?name=" + filename
    );
...
class Tor extends BaseElement {
  ...
  async dhUpdate() {
    ...
    try {
      ...
      data.forEach((obj) => {
        ...
        el.innerHTML = `<td>${obj.Peer.substr(
          0,
          17
        )}</td><td>${obj.Key.substring(0, 21)}</td>`;
  ...
  async create() {
    ...
    const addr = this.peerInfo.getAPIURL(
      "/tor/createCircuit?key=" + finalDestination + "&value=" + val
    );
    ...
    try {
      ...
      this.flash.printSuccess("Circuit created!");
    } catch (e) {
      this.flash.printError("Failed to fetch catalog: " + e);
    }
  }
